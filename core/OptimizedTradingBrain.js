...
console.log(`   📊 EMA9=${ema9?.toFixed(2) || 'null'}, EMA20=${ema20?.toFixed(2) || 'null'}, EMA50=${ema50?.toFixed(2) || 'null'}`);
    ...
    // REFACTOR 2026-02-19: Removed hardcoded regime filter gate.
    // Regime detector ALREADY contributes bearish confidence (+25%) at line 2552 above.
    // Double-punishing by also blocking independently was killing valid trades.
    // The bearish contribution naturally lowers the chance of a BUY clearing MIN_TRADE_CONFIDENCE.
    ...
    // REFACTOR 2026-02-19: Remove hardcoded 0.40 gate. Direction = whoever wins.
    // MIN_TRADE_CONFIDENCE (the ONE tunable threshold) decides if signal is strong enough.
    // Only require a small directional edge to avoid coin-flip entries.
    const minDirectionalEdge = 0.05; // 5% minimum advantage (bull must beat bear by at least 5%)
    ...
    // REFACTOR 2026-02-19: RSI safety only at TRUE extremes (88/12)
    // 80/20 was too aggressive — many valid trades happen at RSI 70-80
    if (marketData && marketData.indicators && marketData.indicators.rsi) {
      if (marketData.indicators.rsi > 88 && direction === 'buy') {
        console.log(`🚫 RSI SAFETY: Blocking BUY at RSI ${marketData.indicators.rsi.toFixed(1)} (>88) - extreme overbought!`);
        signalBreakdown.filters.push({ type: 'RSI_SAFETY', action: 'BLOCKED_BUY', detail: `RSI ${marketData.indicators.rsi.toFixed(1)} > 88` });
        direction = 'hold';
        finalConfidence = 0;
      } else if (marketData.indicators.rsi < 12 && direction === 'sell') {
        console.log(`🚫 RSI SAFETY: Blocking SELL at RSI ${marketData.indicators.rsi.toFixed(1)} (<12) - extreme oversold!`);
        signalBreakdown.filters.push({ type: 'RSI_SAFETY', action: 'BLOCKED_SELL', detail: `RSI ${marketData.indicators.rsi.toFixed(1)} < 12` });
        direction = 'hold';
        finalConfidence = 0;
      }
    }
  ...
  determineTradingDirection(marketData, patterns, confidence) {
    // REFACTOR 2026-02-19: calculateRealConfidence already analyzed ALL indicators,
    // patterns, modules, and set marketData.direction. This method was re-analyzing
    // the same data with DIFFERENT thresholds, creating inconsistency and extra gates.
    // Now it's a simple passthrough that trusts the upstream decision.

    if (!marketData || typeof marketData !== 'object') {
      console.error('❌ determineTradingDirection: Invalid marketData');
      return 'hold';
    }

    // Use direction already determined by calculateRealConfidence
    if (marketData.direction && ['buy', 'sell'].includes(marketData.direction)) {
      console.log(`📊 Direction from confidence calc: ${marketData.direction.toUpperCase()} (Bull: ${(marketData.bullishScore * 100 || 0).toFixed(1)}%, Bear: ${(marketData.bearishScore * 100 || 0).toFixed(1)}%)`);
      return marketData.direction;
    }

    // No direction set or neutral — hold
    console.log(`📊 No directional signal — HOLD`);
    return 'hold';
  }
  ...
  try {
    ...
    // REFACTOR 2026-02-19: Removed 0.15 floor. MIN_TRADE_CONFIDENCE in run-empire-v2.js
    // is the ONE threshold that decides if confidence is high enough. This was redundant.
