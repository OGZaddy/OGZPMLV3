...
const DATA_FILE = path.join(__dirname, 'data', 'polygon-btc-1y-15min.json');
const TEMP_FILE = path.join(__dirname, 'data', 'quick-val.json');

const raw = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
const candles = raw.candles || raw;
fs.writeFileSync(TEMP_FILE, JSON.stringify(candles.slice(0, 4000)));

console.log('Quick validation V3: 4000 15-min candles, 40% threshold, VERBOSE ON...\n');
