...
class SupportResistanceDetector {
  ...
  findLevelsByClustering(candles) {
    ...
    for (let i = candles.length - lookback; i < candles.length; i++) {
      ...
      const highRounded = roundPrice(h(candles[i]));
      const lowRounded = roundPrice(l(candles[i]));
    ...
    for (let i = range; i < candles.length - range; i++) {
      ...
      if (isHighFractal) {
        ...
        for (let j = i + range + 1; j < candles.length; j++) {
          ...
          if (h(candles[j]) > h(candles[i]) * 0.995 &&
              h(candles[j]) <= h(candles[i])) {
            strength++;
          }
        ...
        if (strength >= this.config.fractalStrengthMin) {
          const price = roundPrice(h(candles[i]));
          levels.push({
            price,
            strength: strength,
            method: 'fractal',
      ...
      if (isLowFractal) {
        ...
        for (let j = i + range + 1; j < candles.length; j++) {
          // If price approaches but doesn't break the level, increase strength
          if (l(candles[j]) < l(candles[i]) * 1.005 &&
              l(candles[j]) >= l(candles[i])) {
            strength++;
          }
        ...
        // Only add significant fractals
        if (strength >= this.config.fractalStrengthMin) {
          const price = roundPrice(l(candles[i]));
          levels.push({
            price,
            strength: strength,
            method: 'fractal',
  ...
  findLevelsByVolume(candles) {
    ...
    for (const candle of candles) {
      const midPrice = roundPrice((h(candle) + l(candle)) / 2);
      volumeProfile[midPrice] = (volumeProfile[midPrice] || 0) + v(candle);
    }
